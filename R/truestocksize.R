...
# To do:
  ...
  # search turn on/turn off to find lines that may be switched to allow deteministic projection to unfished biomass
...
truestocksize <- function(Catch = NULL,
  ...
  u_exploit <- u_rec_cat*rec_cat_s + u_rec_disc*rec_disc_s + u_com*com_s # turn off for unfished biomass projection
  # u_exploit <- 0 # turn on for unfished biomass projection
  ...
  u_exploit_tminusone <- u_rec_cat_tminusone*rec_cat_s + u_rec_disc_tminusone*rec_disc_s + u_com_tminusone*com_s # Exploitation rate due to all fleets (u_exploit) # turn off for unfished biomass projection
  # u_exploit_tminusone <- 0 # turn on for unfished biomass projection
  ...
  R_tplusone <- meanR*exp(Rdev_tplusone - 0.5*(R_sigma^2)) # turn off for unfished biomass projection
  # R_tplusone = meanR # Turn on for unfished biomass projection
...
truestocksize(Catch = c(TotRecCatDat[length(TotRecCatDat)], TotRecDiscDat[length(TotRecDiscDat)], ComCatDiscDat[length(ComCatDiscDat)]), # updated
              ...
              OM_R = RData, # updated
              OMR_dev = c(exp(4.89E-01),	exp(7.12E-01), exp(-6.81E-02),	exp(4.43E-01),	exp(4.82E-01),	exp(7.17E-02),	exp(-1.39E+00),	exp(-3.07E-01),	exp(-1.32E-01),	exp(-5.21E-02),	exp(-6.13E-02),	exp(-1.34E-01),	exp(1.51E-01),	exp(4.45E-01),	exp(1.71E-01),	exp(4.30E-02),	exp(8.36E-02),	exp(-1.28E-01),	exp(1.90E-01),	exp(2.62E-01),	exp(3.06E-01),	exp(4.73E-03),	exp(3.59E-01),	exp(-2.01E-01),	exp(-2.63E-02),	exp(5.39E-02),	exp(2.23E-01),	exp(3.88E-01),	exp(2.63E-02),	exp(-4.70E-01),	exp(-3.55E-01),	exp(-3.12E-01),	exp(-1.79E-01),	exp(-5.34E-01),	exp(-3.68E-01),	exp(-1.83E-01))) # log values of recruitment deviations from assess_cor_file.csv
...
for(iyear in 1:50){
  projResult <- truestocksize(Catch = c(0, 0, 0), # updated
                              ...
                              N_tminusone = AbundData[nrow(AbundData)-1,], # updated
                              OM_R = RData, # updated
                              OMR_dev = tempRdevs) # placeholder see line 141 above
  AbundData <- rbind(AbundData, projResult$new_Nabund)
  RData <- projResult$OM_Rs
  tempRdevs <- projResult$OM_Rdevs
}
