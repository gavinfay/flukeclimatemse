...
truestocksize <- function(Catch = NULL,
  ...
  # Exploitation rate due to all fleets (u_exploit)
    ...
    u_exploit <- ifelse(u_exploit>=1,0.999,u_exploit)
  #} else{
  ...
  #increment ages
  new_Nabund <- rep(0,length(N_tilda))
  new_Nabund[-1] <- N_tilda[-(length(N_tilda))]
  new_Nabund[length(N_tilda)] <- new_Nabund[length(N_tilda)] + N_tilda[length(N_tilda)]
  ...
  R_rho <- 0.414 # Fixed input for magnitude of recruitment autocorrelation, see Data_formatting.R
    # Calculations
  delta_tplusone <- rnorm(1, 0, R_sigma) # 1 obs, mean=0, variance, change in recruitment
  Rdev_t <- OMR_dev # Recruitment deviation from most recent year (ε_t)
  Rdev_tplusone <- R_rho*Rdev_t + delta_tplusone*sqrt(1-R_rho^2) # (ε_t+1)
  if(deterministic == "OFF"){
  ...
  new_Nabund[1] <- R_tplusone

  # Calculate OM Biomass in year t+1
  OM_Bio <- sum(w_age*new_Nabund,na.rm = TRUE) # Age zero fish abundance come only from Recruitment, not abundanc
  ...
  return(list(OM_Bio = OM_Bio, # Number for stock biomass in year t+1
              new_Nabund = new_Nabund, # Updated abundance in year t
...
RMean <- 10000
M_age <- c(0.26, 0.26, 0.26, 0.25, 0.25, 0.25, 0.25, 0.24)
Natage <- rep(RMean,length(M_age))
for (age in 2:length(Natage)) Natage[age] <- Natage[age-1]*exp(-1*M_age[age-1])
Natage[length(Natage)] <- Natage[length(Natage)]/(1-exp(-1*M_age[length(Natage)]))
# 1 year projection
yrsCatch <- c(TotRecCatDat[length(TotRecCatDat)], TotRecDiscDat[length(TotRecDiscDat)], ComCatDiscDat[length(ComCatDiscDat)])
#yrsCatch <- rep(0,3)
truestocksize(Catch = yrsCatch, # updated
...
N[1,] <- Natage
w_age <- c(0.001075, 0.022275, 0.110525, 0.282350, 0.527550, 0.824425, 1.149625, 1.483100)
Biomass[1] <- sum(w_age*Natage)
Rdevs <- rep(0,51)
set.seed(42)
for(iyear in 2:51){
  ...
  Rdevs[iyear] <- projResult$OM_Rdevproj
  #Biomass <- rbind(Biomass, projResult$OM_Bio)
  Biomass[iyear] <- projResult$OM_Bio
}
